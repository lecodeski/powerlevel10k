...
local -ra lean_left=(
  '%$frame_color[$color]F╭─ ' '${extra_icons[1]:+%f$extra_icons[1] }%31F$extra_icons[2]$dir_anchor%39F~%b%31F/${dir_full:+$dir_anchor%39Fsrc%b%31F/}${dir_shortened:+%103Fs%31F/}%B%39F$dir_last%b%f $prefixes[1]%76F$extra_icons[3]master%f '
...
local -ra lean_8colors_left=(
  '%$frame_color[$color]F╭─ ' '${extra_icons[1]:+%f$extra_icons[1] }%4F$extra_icons[2]%4F~/${dir_full:+src/}${dir_shortened:+s/}$dir_last%f $prefixes[1]%2F$extra_icons[3]master%f '
...
local -ra classic_left=(
  '%$frame_color[$color]F╭─' '%F{$bg_color[$color]}$left_tail%K{$bg_color[$color]} ${extra_icons[1]:+%255F$extra_icons[1] %$sep_color[$color]F$left_subsep%f }%31F$extra_icons[2]$dir_anchor%39F~%b%K{$bg_color[$color]}%31F/${dir_full:+$dir_anchor%39Fsrc%b%K{$bg_color[$color]\}%31F/}${dir_shortened:+%103Fs%K{$bg_color[$color]\}%31F/}%B%39F$dir_last%b%K{$bg_color[$color]} %$sep_color[$color]F$left_subsep%f %$prefix_color[$color]F$prefixes[1]%76F$extra_icons[3]master %k%$bg_color[$color]F$left_head%f'
...
local -ra rainbow_left=(
  '%$frame_color[$color]F╭─' '%F{${${extra_icons[1]:+7}:-4}}$left_tail${extra_icons[1]:+%K{7\}%232F $extra_icons[1] %K{4\}%7F$left_sep}%K{4}%254F $extra_icons[2]$dir_anchor%255F~%b%K{4}%254F/${dir_full:+$dir_anchor%255Fsrc%b%K{4\}%254F/}${dir_shortened:+%250Fs%K{4\}%254F/}%B%255F$dir_last%b%K{4} %K{2}%4F$left_sep %0F$prefixes[1]$extra_icons[3]master %k%2F$left_head%f'
  '%$frame_color[$color]F╰─' '%f ${buffer:-$cursor}'
)
...
function ask_dir_anchor_first() {
  # The option requires POWERLEVEL9K_SHORTEN_STRATEGY=truncate_to_unique.
  [[ $style == (classic|rainbow|lean*) ]] || return 0
  add_widget 0 flowing -c "%BFirst Directory%b"
  add_widget 0 print
  add_widget 1
  add_widget 0 flowing +c -i 5 "%B(1)  Shortened.%b" All directories between the first and \
    the last are shortened to a unique prefix.
  add_prompt dir_last=p10k dir_shortened=1
  add_widget 0 flowing +c -i 5 "%B(2)  In full.%b" The first directory keeps its name and the \
    color of an anchor.
  add_prompt dir_last=p10k dir_full=1 dir_anchor=
  add_widget 0 print -P "(r)  Restart from the beginning."
  ask 12r
  case $choice in
    r) return 1;;
    1) dir_anchor_first=0; options+='shortened first dir';;
    2) dir_anchor_first=1; options+='full first dir';;
  esac
  return 0
}
...
function generate_config() {
  ...
  (( dir_anchor_first )) && sub DIR_ANCHOR_FIRST true
...
while true; do
  ...
  local -a prefixes=('' '')
  local dir_anchor=%B dir_full= dir_shortened= dir_last=src
  local -a options=()
