...
local -ra lean_left=(
  '%$frame_color[$color]F╭─ ' '${extra_icons[1]:+%f$extra_icons[1] }%31F$extra_icons[2]%B%39F~%b%31F/${dir_full:+%39Fsrc%31F/}${dir_shortened:+%103Fs%31F/}%B%39F$dir_last%b%f $prefixes[1]%76F$extra_icons[3]master%f '
...
local -ra rainbow_left=(
  '%$frame_color[$color]F╭─' '%F{${${extra_icons[1]:+7}:-4}}$left_tail${extra_icons[1]:+%K{7\}%232F $extra_icons[1] %K{4\}%7F$left_sep}%K{4}%254F $extra_icons[2]%B%255F~%b%K{4}%254F/${dir_full:+%255Fsrc%K{4\}%254F/}${dir_shortened:+%250Fs%K{4\}%254F/}%B%255F$dir_last%b%K{4} %K{2}%4F$left_sep %0F$prefixes[1]$extra_icons[3]master %k%2F$left_head%f'
  '%$frame_color[$color]F╰─' '%f ${buffer:-$cursor}'
)
...
function ask_dir_anchor_first() {
  ...
  add_widget 0 flowing +c -i 5 "%B(2)  In full.%b" The first directory keeps its name and the \
    color of an anchor.
  add_prompt dir_last=p10k dir_full=1
  add_widget 0 print -P "(r)  Restart from the beginning."
  ask 12r
  case $choice in
    r) return 1;;
...
while true; do
  ...
  local -a prefixes=('' '')
  local dir_full= dir_shortened= dir_last=src
  local -a options=()
